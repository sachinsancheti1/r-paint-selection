...
Shiny app: pick a target colour (hex) → rank every shade across the Berger, Asian Paints, and Birla Opus catalogs by perceptual closeness (CIEDE2000 in Lab space) → build a shortlist palette with cost-free color matches → export as CSV for ordering site-trial pots.
...
- A related, easy-to-miss consequence: `data.table::fread()` infers an **entirely blank** character column (Asian's now-empty `name`) as `logical`, not `character` — silently breaking `rbindlist()` when combined with Berger's character-type `name` column. Fixed by forcing `colClasses = c(code = "character", name = "character")` on all `fread()` calls.

**`catalog_birla_opus.csv` (2322 shades)** — added as a third brand. Unlike Berger/Asian (extracted by hand from a spreadsheet), Birla Opus's individual shade pages (`birlaopus.com/colour-catalogue/.../<shade-slug>`) publish real official per-shade data (shade code, name, RGB, LRV) client-side via JS. No bulk export or public API exists (their `robots.txt` explicitly disallows `/graphql/` and the content API), so the full catalogue was extracted by visiting all 2322 individual public shade pages with a headless browser, respectfully paced (300-800ms between page loads, one browser context reused throughout) — the disallowed endpoints were never touched. Verified independently after extraction: 2322/2322 rows, zero duplicate codes, zero empty fields, zero out-of-range RGB values, code format consistent (`<2-letter family prefix> <4-digit number>`, e.g. `WW 0005`) across every row, and a manual spot-check (`WW 0005` / "White linen" / RGB `238,237,233`) matched the live site exactly.
